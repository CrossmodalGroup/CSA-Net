...
class Transformer(BaseCaptioner):
    ...
    def get_bs_device(self, samples):
        if isinstance(samples, dict):
            key = 'rel_feat' if 'rel_feat' in samples else 'reg_feat'
    ...
    def forward(self,
        ...
        if not use_beam_search:
            if not self.cached_features:
                vis_inputs = self.detector(images) #grid feature: 48*1024; region feature:150*512
            else:
                vis_inputs = images

            if self.config.model.use_csa_net:
                vis_inputs['reg_feat'], vis_inputs['rel_feat'], vis_inputs['rel_mask'] = self.csa_net(vis_inputs)
    ...
    def step(self, timestep, prev_output, samples, seq, mode='teacher_forcing', **kwargs):
        ...
        if mode == 'teacher_forcing':
            ...
            if timestep == 0:
                ...
                if self.config.model.use_csa_net:
                    self.reg_feat, self.rel_feat, self.rel_mask = self.csa_net(vis_inputs)

                # If t = 0, enc_output = [B, N, D], init_tokens = [B, 1] t=0,beamsize=1,
                # Else t > 0, enc_output = [BB, N, D], it = prev_output (t-1) = [BB, 1] t>0,beamsize=5
                _feat = getattr(self, 'rel_feat', self.reg_feat)
