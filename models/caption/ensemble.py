...
class Ensemble(nn.Module):
    ...
    def __init__(
        ...
        self.use_rel_feat = use_rel_feat
        self.config = config

    def get_bs_device(self, samples):
        if isinstance(samples, dict):
            batch_size = samples['rel_feat'].shape[0]
            device = samples['rel_feat'].device
    ...
    def _expand_state(self, selected_beam, cur_beam_size, batch_size, beam_size):
        ...
        #         beam = beam.unsqueeze(-1)
        #     tensor = torch.gather(
        #         tensor.view((batch_size, cur_beam_size) + tuple(shape[1:])),
        #         1,
        #         beam.expand((batch_size, beam_size) + tuple(shape[1:]))
        #     )
        #     tensor = tensor.view((-1,) + shape[1:])
        #     return tensor
        ...
        for model in self.models:
            if timestep == 0:
                if not model.cached_features:
                    ...
                    vis_inputs = samples
                #model.rel_feat, model.rel_mask = model.encoder(vis_inputs['rel_feat'], vis_inputs['rel_mask'])
                model.reg_feat, model.rel_feat, model.rel_mask = model.csa_net(vis_inputs)
                # if model.config.model.decoder.decoder_name != 'mesh':
                #     model.rel_feat = model.rel_feat[:, -1]

                if model.config.model.use_reg_feat:
                    model.reg_feat = vis_inputs['reg_feat']
                ...
                it = model.rel_feat.data.new_full((model.rel_feat.shape[0], 1), model.bos_idx).long()
            else:
                it = prev_output

            vis_inputs = {
                'rel_feat': model.rel_feat,
                'rel_mask': model.rel_mask,
            }
            if model.config.model.use_reg_feat:
                vis_inputs['reg_feat'] = model.reg_feat
                vis_inputs['reg_mask'] = model.reg_mask
            step_outputs.append(model.cap_generator(it, vis_inputs))
