...
def main(gpu, config):
    ...
    if config.model.use_csa_net:  #csa_net
   ...
   # tokenizer = multiprocessing.Pool(1)  #config.optimizer.num_workers)
    ...
    for epoch in range(max(0, start_epoch), total_epochs):
        ...
        if phase == 'fr_xe' or phase == 'ft_xe':
            ...
            cider = Cider(PTBTokenizer.tokenize([e.text for e in train_dataset.examples]))
            tokenizer = multiprocessing.Pool(4)
...
@hydra.main(config_path="configs/caption", config_name="coco_config_csa_net_end_to_end")# , version_base=None)
